...
import numpy as np
import random
from torch_network import policy_network as policy_network
from torch_network import critic_network as critic_network
import torch.nn.functional as F
import copy
import torch.multiprocessing as mp
from data_generating_SP import MRI
from data_generating_SP import test
import random
...
def run_env(ppo,ep):
    ppo.start_epoch(1)
    obs,_=ppo.env.reset(rest=ppo.rest)
    total_reward=0
    total_reward2=0
    total_reward3=0
    I=1
    ...
    for i in range(ppo.long_time):
        with torch.no_grad():
            ...
            angle=ppo.action(obs1,r,old_act)
            r -= 1
            next_obs,reward,done,_=ppo.env.step(angle)
            ...
            p1,advantage,value=ppo.process(obs1,r+1,angle,next_obs1,r,reward1,done)
            ppo.next_hidden(obs1,r+1)
            history.append(((obs1.copy(),r+1),(angle,p1,advantage,value,reward,obs.copy()),reward1,done,(next_obs1.copy(),r)))
...
class PPO_RB():
    def __init__(self,size,rest,sample_size,hidden_cell,n_layers,method="ridge",actor_lr=1e-5,
                 critic_lr=1e-5,gamma=0.99,use_linear_lr_decay=True,save_path='/burg/apam/users/yz3888/ctmodel/',
                 load_path='/burg/apam/users/yz3888/ctmodel/',num_process=3,max_length=100,shape="oval"):
        self.reward_scale=None
        self.env=MRI(size,rest,sample_size,shape=shape)
        self.state_dim = int(self.env.img_data[0].shape[0] * self.env.img_data[0].shape[0])
    ...
    def gae(self,history,lam):
        ## history = ((state),(angle,p1,advantage,value,reward,obs),reward1,done,(next_state))
        lens=len(history)
        result_history=[]
        gae=0
        rs=0
        ...
        for i in reversed(range(lens)):
            ...
            rs=self.gamma*rs+history[i][1][3]
            j+=1
            self.runningstat.push(rs)
            observe=history[i][1][5].tolist()
            for data in observe:
            ...
            result_history.append((history[i][0],(history[i][1][0],history[i][1][1],history[i][1][2],history[i][1][3]),
                                   history[i][2],history[i][3],history[i][4]))
        ## result_history = (state,(angle,p1,gae,value,reward),reward1,done,next_state)
        result_history.reverse()
        return result_history

    '''use policy network and target critic network to compute estimated advantage and value '''
    def process(self,obs,r1,angle,next_obs,r2,reward,if_end):
        ...
        with torch.no_grad():
            ...
            p1,_,_=self.policy_net.prob(state,self.hidden_a_pre,angle1)
    ...
    def train(self,batch_size):
        ...
        for i in range(self.rest):
            ...
            ## history[i] = (state,(angle,p1,gae,value,reward),reward1,done,next_state)
            ...
            for j in range(batch_size):
                if i<len(batch_train[j]):
                    ...
                    ## advantage = gae 
                    advantage.append(batch_train[j][i][1][2])
                    value.append(batch_train[j][i][1][3])
        ...
        self.a=0.3
        p1,_,_= self.policy_net.prob(pre_state,self.hidden_a_pre,action)
        r=(p1)/(p1_old)
    ...
    def test_train(self,time_len,deterministic=True):
        ...
        for i in range(self.long_time):
            ...
            angle=self.action(obs1,rest,old_act,deterministic)
            rest-=1
    ...
    def action(self,state,rest,choose_act,deterministic=False):
        ...
        with torch.no_grad():
            angle = self.policy_net.act(state,self.hidden_a_pre,choose_act,deterministic)
        return angle
    ...
    def save(self, save_path,ep):
        ...
        stat_fp=open(save_path+'obs_stat.txt','wb')
    ...
    def plot_freq_prob(self,ff,ep):
        ...
        for pos in range(self.rest):
            a_list=[]
            for i in range(pos,len(self.freqprob),self.rest):
                s += self.freqprob[i][0][0][ff]/self.num_process
                m += 1
                if m == self.num_process:
                    m = 0
                    a_list.append(s)
                    s = 0
            if self.rest ==10:
                ...
                axs[pos//3,pos%3].plot(a_list)
                axs[pos//3,pos%3].set_title("Sensor"+str(pos))
        fig.savefig(self.save_path+"_"+str(ep)+"_"+"freq"+str(ff)) 
        plt.close()   
        
        
agent=PPO_RB(size=2,rest=1,shape="circle",sample_size=1,num_process=1,
             hidden_cell=(256,64),n_layers=(3,2),save_path='/burg/apam/users/yz3888/ctmodel/',load_path="")
agent.multi_train(1)