...
from data_generating_nonlinear3 import IS
from data_generating_nonlinear3 import test
import random
...
class PPO_RB():
    def __init__(self,size,rest,freq_bound,sample_size,state_dim,action_dim,hidden_cell,n_layers,num_freq=4,method="ridge",actor_lr=1e-5,
                 critic_lr=1e-5,gamma=0.99,use_linear_lr_decay=True,save_path='../ctmodel/',
                 load_path='../ctmodel/',num_process=3,max_length=100,shape="oval"):
        ...
        self.freqprob=[]
        img_path='../ct_data'
        self.env=IS(size,rest,num_freq,sample_size,shape=shape)
        self.runningstat=RunningStat(())
        self.statestat=RunningStat(())
        '''stat_fp=open(save_path+'obs_stat.txt','rb')
...
agent=PPO_RB(size=32,rest=10,shape="circle",sample_size=1000,num_process=3,freq_bound=32,num_freq=4,state_dim=20,action_dim=360,
